...
int main(int argc, char **argv) {
    ...
    while (fgets(cmd, CMD_LEN, stdin) != NULL) {
        ...
        if (strcmp(first_token, "pwd") == 0) {
            ...
            // 1. check the length of token
            // if the length 1, then change to home directory 
            if (tokens.length == 1) {
        ...
        // Task 6: Wait for all background jobs
            ...
            if (child_pid == 0) {
                // 2. call run_command() in the child process
                run_command(&tokens);
                // 3. exit the child process
                exit(0);
            } else {
                if (foreground) {
                    // 4. wait for the program to exit
                    int child_status;
                    // Set the child process as the target of signals sent to the terminal via the keyboard using tscsetpgrp
                    // tcsetpgrp returns the process group ID of the foreground process group of the terminal associated to fd(file descriptor)
                    ...
                    // Return shell to foreground after child terminates
                    if (tcsetpgrp(STDIN_FILENO, getpid()) == -1) {
                        perror("tcsetpgrp");
                    }   
                    // If the child process has stopped, add it to the job list
                    // WIFSSTOPPED returns true(1) if the child process has stopped
                    if (WIFSTOPPED(child_status)) {  
                        job_list_add(&jobs, child_pid, first_token, JOB_STOPPED);
                    }
                } else {
                    job_list_add(&jobs, child_pid, first_token, JOB_BACKGROUND);
                }
